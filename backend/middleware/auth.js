...
const protect = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'myfin_secret');
    if (decoded.role === 'ADMIN') {
      ...
      if (customer.status !== 'ACTIVE') return res.status(403).json({ success: false, message: 'Account is not active' });
      req.user = customer;
      req.role = 'CUSTOMER';
    }
    next();
...
const customerOnly = (req, res, next) => {
  if (req.role === 'CUSTOMER') return next();
